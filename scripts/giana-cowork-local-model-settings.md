...
Both admitted local catalogs use `availabilityProbe.phase: dispatch`. The application can therefore render GLM official and an already configured `qwen-local-r5300` without requiring either listener at startup. The Qwen route is normalized to the exact admitted model with a truthful 32,768-token deployment window and a 4,096-token output cap; the smaller common contract keeps pressure compaction aligned across R5300 and PRDG instead of advertising the model artifact's larger theoretical capacity. The lifecycle manager runs first when a user dispatches to a cold route; only after a successful load/health/probe sequence does the adapter send the chat request. A failed lifecycle dispatch remains a model-unavailable result and must not fall through to a mismatched provider.
