...
try {
  ...
  const chip = page.locator('[data-annotation-attachment] [data-annotation-count="1"]')
  ...
  check('one compact annotation chip', await chip.isVisible() && (await chip.innerText()) === '1 annotation', await chip.innerText())
  ...
  check('hover shows selected source text', (await preview.innerText()).includes(quote), await preview.innerText())
  await page.screenshot({ path: join(output, 'desktop-chip-hover.png'), fullPage: true })
  await page.mouse.move(890, 740)
  await preview.waitFor({ state: 'detached' })
  await chip.focus()
  await preview.waitFor()
  check('keyboard focus shows source text', (await preview.innerText()).includes(quote), await preview.innerText())
  ...
  const mobile = await page.evaluate(() => {
    const chip = document.querySelector('[data-annotation-attachment] [data-annotation-count="1"]')?.getBoundingClientRect()
  ...
  check('mobile preview fits viewport', mobilePreview !== null && mobilePreview.x >= 0 && mobilePreview.x + mobilePreview.width <= 390, mobilePreview)
