...
| `@deepseek-ai/dsh-tool-skill` | `skill`, `skill_search` | `ctx.tools`, `ctx.agents`, `ctx.skills` | `tool/call`, `tool/result`, `user/message replacement catalogs via agent.inject()` | - | - |
...
Search every available model-invocable skill by name and description. Use this when the bounded session catalog does not list the capability you need, then call `skill` with an exact returned name.
...
```json
{
  ...
  "properties": {
    "query": {
      "type": "string",
      "description": "Words describing the capability or skill name to find."
    }
  },
  "required": [
    "query"
