...
/** Display options for the Giana Code Putri wordmark. */
export interface BrandWordmarkProps extends IconProps {
  /** Whether to include the leading Giana OS mark; defaults to true. */
  includeMark?: boolean | undefined
}

/** Render the public Giana Code Putri product mark. */
export function BrandWordmark({ size = 24, className, includeMark = true }: BrandWordmarkProps) {
  return (
    <span
      className={className}
      aria-label="Giana Code Putri"
...
function GianaMark({ size }: { size: number }) {
  return <img src="/giana-os-logo.png" width={size} height={size} alt="" aria-hidden="true" style={{ objectFit: 'contain' }} />
}
