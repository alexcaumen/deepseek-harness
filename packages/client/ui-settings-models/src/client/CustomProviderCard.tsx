...
export function CustomProviderCard(props: CustomProviderCardProps): ReactNode {
  ...
  // Reuse registered copy so this backport needs no locale dictionary changes.
  const baseUrlError = `${t('customNeedsBaseUrl')} (HTTP/HTTPS)`
  ...
  return (
    <div className={styles['editor']}>
      ...
      {baseUrlInvalid ? <p className={styles['error']}>{baseUrlError}</p> : null}
      <div className={styles['field']}>
      ...
      <ModelListEditor
        ...
        api={api}
        t={key => key === 'customNeedsBaseUrl' ? baseUrlError : t(key)}
        disabled={profileDisabled}
      />
