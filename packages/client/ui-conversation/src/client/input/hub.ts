...
export class InputHub implements SessionInputResolver {
  ...
  shellFor(binding: SessionBinding): SessionInputShell {
    ...
    const shell = new SessionInputShell({
      ...
      defaultSink: (text, imageIds, mode, signal, displayText) => (
        this.sink(session, text, imageIds, mode, signal, displayText)
      ),
  ...
  private sink(
    ...
    return this.conversation().sendSession(session, text, imageIds, mode, signal, displayText)
  }

  /**
   * Steer every still-pending queued message into the running turn, in FIFO
   * order — the same strict-steer operation as the queue dock's per-row
   * button. A turn closing mid-way (`steer-unavailable`) stops the flush.
   * A row already claimed by the agent (`queue-item-not-found`) is skipped so
   * later still-pending rows can be steered. A genuine failure surfaces as
   * one composer notice.
  ...
  private async steerQueue(session: SessionFace, shell: SessionInputShell): Promise<void> {
    ...
    for (const item of queued) {
      ...
      if (result.error.code === 'steer-unavailable') return
