...
describe('automatic-language dictation', () => {
  ...
  it('makes quiet speech visible and settles silence back to the waveform floor', async () => {
    ...
    act(() => { frames.step(16) })
    expect(bars[71]?.style.getPropertyValue('--dictation-amplitude')).toBe('0.309')
    act(() => { frames.step(120) })
    expect(bars[70]?.style.getPropertyValue('--dictation-amplitude')).toBe('0.309')
    expect(bars[71]?.style.getPropertyValue('--dictation-amplitude')).toBe('0.183')
