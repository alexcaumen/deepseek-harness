...
The expanded brand row renders `sidebar.brand.mark` and `sidebar.brand.name` as independent single slots with a 64px mark in an 80px row; the collapsed rail requests a 32px mark. Without occupants, the shell uses the user-provided logo and the Giana CoWork Preview wordmark, with the build's 7-character `DSH_CLIENT_COMMIT_HASH` badge below it. The name and Preview line wrap without shrinking the logo. A deployment package can replace either slot without replacing the New Session control or rail geometry; declaration-aware `slots.inject()` lets such a package activate before or after the sidebar.
