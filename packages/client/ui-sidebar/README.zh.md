...
展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，在 80px 行中显示 64px 标志；收起轨道请求 32px 标志。没有占位者时，外壳使用用户提供的标志和 Giana CoWork Preview 字标，下方保留构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标。名称和 Preview 行可换行而不缩小标志。部署包可以单独替换任一 slot，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
