 ...
 * except workspace Rename/Delete and session Rename/Fork/Archive/Unarchive; the session
 ...
 * @param props.archived - render the archived row with only its restore action.
...
export function SessionNodeItem({
  ...
  /** Unarchive this session (the only archived-row menu action). */
  onUnarchive?: ((id: SessionNode['id']) => void) | undefined
  /** Render outside active group/order accounting in the archived section. */
  ...
  const selected = !archived && node.id === currentId
  const statuses = sessionStatuses(node, t)
  const primaryStatus = statuses[0]
  ...
  const ownRow = (
    <div
      ...
      draggable={!archived && drag !== undefined}
      onDragStart={drag === undefined
        ? undefined
        : (e) => {
          e.dataTransfer.effectAllowed = 'move'
