...
子 agent 会获得父 agent 的工作目录／会话谱系；除非 `request.agentOptions` 覆盖，否则还会继承父 agent 最近一次已组装请求的提供方、模型和输出 token 上限（首次请求之前回退到创建选项）。这样，模型进行委派时，UI 为会话选择的路由不会丢失。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
