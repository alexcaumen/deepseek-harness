...
The child gets the parent's working-directory/session lineage and inherits the provider, model, and output-token cap from the parent's latest assembled request (falling back to its creation options before the first request) unless `request.agentOptions` overrides them. This keeps a UI-selected session route intact when that model delegates. It gets a fresh flat registration scope: parent ownership does not import parent tool restrictions or establish an authority subset.
