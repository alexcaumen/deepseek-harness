...
The child gets the parent's working-directory/session lineage and inherits the provider and model from the parent's latest resolved request context, plus the output-token cap from its latest assembled request (falling back to creation options before the first request), unless `request.agentOptions` overrides them. This keeps a UI-selected session route intact when that model delegates. It gets a fresh flat registration scope: parent ownership does not import parent tool restrictions or establish an authority subset.
