...
The provider advertises one-shot `agentOptions` support and reports `inheritsParentContext: false`. A non-empty per-run `agentOptions.model` overrides the configured model only for that child and is sent as official `thread/start.model`; a non-empty `agentOptions.reasoningEffort` is sent as official `turn/start.effort`. Omitting either field preserves the configured model fallback or native Codex effort default, and concurrent runs keep their selections isolated. `agentOptions.provider` is validated as non-empty route metadata but native Codex remains the transport authority. Codex receives the standalone text task and the parent Session cwd, but not the parent conversation, persona, tool filter, depth policy, structured-output contract, or parent credentials. The ephemeral Codex thread id and turn id stay private to this run and are never persisted in the parent Session.
...
Production resolves the `codex` bin declared by its pinned `@openai/codex@0.147.0` dependency and launches that JavaScript wrapper with the current Node executable. The wrapper selects the matching native platform payload; the provider neither inspects nor falls back to a host `codex` on `PATH`. Native Codex configuration and authentication remain authoritative through the parent cwd, `HOME`, and `CODEX_HOME`. The effective one-shot model is the request override, then the configured `model`, then native Codex settings; only the selected value is sent on that run's ephemeral `thread/start`. A requested reasoning effort is sent only on that run's `turn/start`; omission preserves the native default. The Provider does not discover models, rewrite aliases, select `modelProvider` or `serviceTier`, or retain a request override after the run. The selected permission mode remains the only other thread-level override. The plugin does not create `CODEX_HOME`, log in, probe an account, or log route values or credentials. Credential-shaped ambient variables are removed by the subprocess seam before the explicit `env` overlay is applied.
...
The Codex child receives the standalone text blocks as one turn in a fresh ephemeral thread. Its workspace is the parent Session cwd; its model comes from the one-shot request override, configured Provider fallback, or native Codex settings in that order. A one-shot reasoning effort overrides the native default for that turn only. System instructions, tools, and authentication remain native Codex concerns. The selected Provider instance's Profile configuration fixes the thread's environment, non-interactive approval policy, and sandbox mode, and the executable version comes from the Bundle's pinned platform payload.
