...
- **Cross-process write exclusion.** Before append or repair, the coordinator takes a non-blocking kernel lease for the session: `flock` on POSIX and a named semaphore derived from the canonical artifact path on Windows. A concurrent live writer is rejected. Session retirement and backend teardown drain writes before releasing the lease; process death releases it in the kernel, so takeover has no stale-file delay.
