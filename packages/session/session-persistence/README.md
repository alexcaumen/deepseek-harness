...
When a live session emits `session/disposed`, the coordinator waits for its controller, serializes a final drain, then releases state and any backend write lease owned by that exact `Session` object. Failed retirement leaves the controller in the live-session map, so backend teardown can retry it. Backend teardown stops event admission first, flushes every remaining controller, awaits per-id operations, releases retained leases, and only then closes the storage handle.
...
| `acquireWriteLease?(meta)` | Optionally take exclusive cross-process write ownership. The coordinator rechecks `readStoredRevision` while the lease is held, retains ownership across appends, and releases it after live retirement or backend teardown. |
