...
| `llm/dispatch-signal` | waterfall | 流准入后添加传输取消信号，不改写模型请求 |
...
- 使用 `llm/dispatch-signal` 可在流准入后、最终适配器解析及派发前添加资源生命周期取消信号。调用 `next()` 并合并其结果。原始调用者信号始终保留；只有独立的传输请求副本获得合并信号。冻结请求、消息内容和预备适配器身份保持不变。适配器必须响应取消；取消不等于远程工作已经停止。
