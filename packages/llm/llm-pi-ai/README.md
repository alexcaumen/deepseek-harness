...
`availabilityProbe` is an opt-in readiness gate for hand-declared OpenAI-compatible routes. It accepts an exact `model`, a bounded `timeoutMs`, and `phase`. The default `eager` phase checks `/models` before catalog listing, selection, and dispatch. `phase: dispatch` keeps an admitted cold route visible in selectors so the composed lifecycle controller can start it, then checks `/models` immediately before the provider request. A failed dispatch probe never sends the chat request.
