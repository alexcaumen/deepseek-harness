...
- `ctx.tools.schemasForRequest(schemas, agent, provider)` applies opt-in native discovery after the caller resolves its provider, before it logs/freezes the request header. `onDemand` config declares exact `providers`, pinned `alwaysAvailable` names, `maxSearchResults`, and `maxActiveTools`. `tool_search` returns currently scoped schemas; successful correlated session results restore recently discovered names. Other providers keep their complete schemas, and execution permissions never change. The default agent loop owns this final projection. See the [GCP decision](../../../.agents/notes/proposed/bug-fix/2026-09-06-gcp-native-tool-discovery.md).
...
External tool consumers use `wireSchemas(scope).schemas` for directly callable tools and `codeSdk(scope)` for the canonical generated program declarations. Both read the current scoped presentation and visibility; code presentations require a supported runtime. `wireSchemas` returns detached schema data; its `knownNames` field is only for prompt-order validation, never a call allowlist. `schemas(scope)` remains the full capability catalog used to construct program bindings. The authenticated [MCP bridge](../../mcp/mcp-server-tool-runtime/README.md) uses these projections without changing execution authority.
