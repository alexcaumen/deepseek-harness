...
系统提示词组装注册表。插件可以贡献有序段、工具 schema 和具名变量。循环在每个步骤组装一次，并将结果渲染为完整的模型提示词。此插件拥有静态产品身份和全局部署 persona；agent（智能体）作用域的 persona 会遮蔽全局默认值。

## 配置

| 键 | 默认值 | 含义 |
|---|---|---|
| `includeHarnessIdentity` | `true` | 是否包含下文所示、顺序为 −100 的固定 Giana CoWork Preview 开场白。仅当兼容性部署拥有完整系统提示词时设为 false。 |
...
- `PromptSection`：`{ name, order, text, complete? }`。各段按 `order` 升序拼接。顺序区间：`-100` 是产品身份（`harness:identity`），`0` 是部署 persona，工具引导使用 `100–199`。协作式组装完成后，一个有效的 `complete` 段会抑制其他所有段。
