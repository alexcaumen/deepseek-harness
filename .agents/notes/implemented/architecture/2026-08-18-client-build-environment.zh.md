...
修改工作区的 HMR 测试会保存完整的 watcher 输出链，包括静态壳、动态 bundle、关联库与编译器增量状态。它先停止写入进程，再恢复源文件和产物字节，仅移除新生成的文件，并重新验证原摘要。只恢复动态 bundle，或在 watcher 仍活动时恢复源码，可能使后续消费者遇到摘要不匹配或不完整的壳。该回归测试导入真实 HMR runner，因此属于 Host 测试组，不能进入 Client 编译器。
