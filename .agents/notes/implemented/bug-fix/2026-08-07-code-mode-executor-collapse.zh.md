...
能力目录包含在 `code` 下只能由嵌套程序调用的工具。把它们通告为可直接调用会产生可预期的 `UNKNOWN_TOOL` 错误。经过认证的 MCP 桥接在列举和分发时都读取公开的 `wireSchemas(scope).schemas` 投影，并把规范的 `codeSdk(scope)` 声明附加到传输工具描述。若只返回传输 schema 而不附 SDK，远程模型就无法发现程序绑定。这两个读取接口共享注册表的作用域模式及语言解析；完整系统提示词组装不是授权来源，还可能执行无关提供方。适配器不会伪造 parent token 或改变 agent 模式以强行执行原生调用。
