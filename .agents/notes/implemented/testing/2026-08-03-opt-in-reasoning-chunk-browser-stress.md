...
The collapsed Think row no longer follows reasoning text horizontally. It projects only running or completed status until the user expands the body, as owned by [Giana Code Putri interaction controls](../feature/2026-09-02-giana-code-putri-interaction-controls.md). Frame-coalesced session publication remains independent of that presentation decision and continues to bound React snapshot work.
...
This decision is partially superseded only for the former horizontal Think-summary scheduler. The frame-coalesced publication mechanism, raw-event fidelity, deterministic scheduling coverage, and opt-in browser stress lane remain current. The collapsed summary now avoids horizontal layout reads and writes entirely and reports status until expansion.
