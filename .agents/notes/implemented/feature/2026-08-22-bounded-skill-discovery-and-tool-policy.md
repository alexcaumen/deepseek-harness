...
GCP's `standard`, `code`, and `cordis` presets pin `j-space` first because their
instructions require loading it before non-trivial work. The catalog remains
limited to 12 summaries; this does not install a missing skill or expand its
permissions. Full-catalog search and exact-name loading remain available.
...
**Require J-Space only in the prompt.** Insufficient: a bounded initial catalog
can omit the required skill even when its provider has registered it. Pin its
summary without injecting every skill body or bypassing provider scope.
